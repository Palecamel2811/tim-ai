...
export default function RecordScreen() {
  ...
  async function startRecording() {
    ...
    function draw() {
      ...
      const vals = Array.from(dataArr).map(v => Math.max(3, (v / 255) * 100))
      setBars(vals)
  ...
  async function stopRecording() {
    ...
    if (blob) sessionStorage.setItem('tim_recording_url', URL.createObjectURL(blob))
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 gap-8 fern-bg">
      <div className="relative z-10 flex flex-col items-center gap-8 w-full max-w-xs">

        {/* Sphere — reacts to mic amplitude */}
        <TIMSphere
          state={status === 'recording' ? 'listening' : 'idle'}
          amplitude={amplitude}
          size={160}
        />
          ...
          {bars.map((h, i) => (
            <div key={i} className="rounded-full transition-all duration-75"
              style={{
                width: '5px',
                height: `${h}%`,
                backgroundColor: status === 'recording' ? '#B8C8E8' : '#2A3550',
                opacity: status === 'recording' ? 0.8 : 0.4,
              }}
            />
          ))}
        </div>
        ...
        {status === 'recording' && notes.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap justify-center">
            <span className="text-[#6A7A9A] text-xs uppercase tracking-widest">TIM hears:</span>
            {[...new Set(notes.map(n => n.note))].slice(-6).map((n, i) => (
              <span key={i} className="text-xs bg-[#B8C8E8]/10 text-[#B8C8E8] px-2 py-0.5
                                       rounded-full font-mono border border-[#2A3550]">
                {n}
              </span>
            ))}
          </div>
        ...
        {/* Timer */}
        <span className="text-[#B8C8E8] font-mono text-2xl tabular-nums tracking-widest">
          {fmt(seconds)}
        </span>
        ...
        <div className="relative flex items-center justify-center">
          ...
          <button
            ...
            className={`relative w-20 h-20 rounded-full flex items-center justify-center
              ...
              ${status === 'recording'
                ? 'bg-[#E05870] scale-110 knob-active'
                : status === 'done'
                ? 'bg-[#1A2235] cursor-not-allowed knob'
                : 'bg-[#B8C8E8]/15 border border-[#B8C8E8]/30 hover:bg-[#B8C8E8]/25 knob'
              }`}
          >
            {status === 'recording'
              ? <Square size={26} className="text-white" />
              : <svg width="26" height="26" viewBox="0 0 24 24" fill="none"
                  stroke={status === 'done' ? '#3A4A6A' : '#B8C8E8'}
        ...
        <p className="text-[#3A4A6A] text-xs text-center">
          {status === 'idle'      && 'Tap and hum a melody, sing a rhythm, or make any sound'}
        ...
        {status === 'done' && (
          <button onClick={() => navigate('/create')}
            className="flex items-center gap-2 px-8 py-3 bg-[#B8C8E8]/10 border border-[#B8C8E8]/20
                       text-[#F0F4FF] font-medium rounded-full hover:bg-[#B8C8E8]/20 transition-all knob">
